...
const AssessmentManager = {
  ...
  renderQuestions: () => {
    ...
    questions.forEach((question) => {
      ...
      let optionsHtml = '<div class="radio-group-horizontal">';
      question.options.forEach((opt, index) => {
        const optionValue = index + 1;
        const isChecked = selectedValue === optionValue ? 'checked' : '';
        optionsHtml += `
    ...
    container.querySelectorAll('.scale-radio').forEach(radio => {
      radio.addEventListener('change', AssessmentManager.onResponseChange);
    });
  },
  ...
  submitAssessment: async (event) => {
    ...
    try {
      ...
      if (isBackendAvailable) {
        ...
        result = await APIService.fallbackToLocal(AssessmentManager.responses);
