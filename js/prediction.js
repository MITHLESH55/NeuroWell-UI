...
const PredictionEngine = {
  /**
   * Create a simple wellness forecast based on current score
   * @param {number|null} score - Current overall wellness score
   * @returns {{day7:number,day14:number,day30:number}}
   */
  generatePrediction: (score = null) => {
    const currentScore = typeof score === 'number'
      ? score
      : StorageManager.getWellnessScore()?.scores?.overall ?? 50;

    return {
      day7: Math.min(100, currentScore + 5),
      day14: Math.min(100, currentScore + 10),
      day30: Math.min(100, currentScore + 15)
    };
  },
  ...
  predict30DayTrajectory: () => {
    ...
    const baseScore = currentScore?.scores?.overall ?? 50;
    ...
    for (let day = 0; day <= 30; day++) {
      const predictedScore = Math.min(
        100,
        Math.max(0, baseScore + (velocity * day * 0.7))
      );
